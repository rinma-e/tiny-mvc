    ...
    HierarchySelect.prototype = {
        ...
        setSelected: function (a) {
            if (a.length && this.previouslySelected !== a) {
                ...
                var allowedClassesForButton = ["btn", "btn-outline-secondary", "text-start", "dropdown-toggle", "show"];
                var buttonClassList = this.$button.attr("class").split(" ");
                var aClassList = a.attr("class").split(" ");
                aClassList.splice(0, 1);
                var classListToRemove = [];
                $.each(buttonClassList, function (index, item) {
                    if (allowedClassesForButton.indexOf(item) == -1) {
                        classListToRemove.push(item);
                    }
                });
                var btn = this.$button;
                if (classListToRemove.length > 0) {
                    $.each(classListToRemove, function (index, item) {
                        if (buttonClassList.indexOf(item) != -1) {
                ...
                this.$button.addClass(aClassList.join(" "));
